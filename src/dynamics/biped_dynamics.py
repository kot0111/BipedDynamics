...
# class PhaseTrajectory:
#     def __init__(self, dyn: BipedDynamics, constr : Constraints):
#         self.dynamics = dyn
...
#         def hit_ground(t, y):
#             return np.abs(y[0]) + self.constraints(y[0])[0]
#         hit_ground.terminal = True
#         hit_ground.direction = 1
...
#             th,dth = st
#             B_perp = [1, 0, 0]
#             alpha, beta, gamma, u = self.constraints._abgu_coeffs(th, self.constraints(th))
#             alpha = B_perp @ alpha
#             beta = B_perp @ beta
#             gamma = B_perp @ gamma
#             ddth =  (-gamma - beta * dth**2) / alpha
...
#         initial_state = self.constraints.initial_state
...
#         t, path = self.reduced_dynamics()
#         th = path[0]
#         dth = path[1]
#         ddth = path[2]
#         q2 = np.array([self.constraints(ph)[0] for ph in th])
#         q3 = np.array([self.constraints(ph)[1] for ph in th])
#         dq2 = np.array([self.constraints(ph)[2] * dph for ph, dph, _ in zip(*path)])
#         dq3 = np.array([self.constraints(ph)[3] * dph for ph, dph, _ in zip(*path)])
#         ddq2 = np.array([self.constraints(ph)[2] * ddph + self.constraints(ph)[4] * dph**2 for ph, dph, ddph in zip(*path)])
#         ddq3 = np.array([self.constraints(ph)[3] * ddph + self.constraints(ph)[5] * dph**2 for ph, dph, ddph in zip(*path)])

#         self.t = np.copy(t)

#         self.theta_sp = sp.interpolate.make_interp_spline(th, path.T[:, 1:3], k=5)
